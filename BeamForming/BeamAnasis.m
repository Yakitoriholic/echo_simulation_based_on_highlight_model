...
% 时延波束形成
beamformed_signals_time_domain = zeros(length(angles), fft_len); % 初始化时域波束形成信号矩阵
phi = 0; % 俯仰角
...
for a = 1:length(angles)
    ...
    pve_B0 = beamscp(ksc, P_r, theta0, phi);
    BF = sum(conj(pve_B) .* pve_B0,1);
    % delay = (i-1)*d*sind(theta)/c; % 计算时延
    % delay_samples = round(delay * fs); % 时延转换为样本数
    % element_signal = apply_delay(element_signals(i,:), delay_samples);
    element_signal_fft = fft(signal_padded, fft_len, 2); % 计算FFT
    beamformed_signal_freq_domain = BF ;
...
save('BD.mat', 't_total', 'beamformed_signals_time_domain');